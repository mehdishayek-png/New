...
// ---- Fetch Internal Scraping API (V2 REPLACEMENT) ----
async function fetchSerpInternal(queries, location) {
  console.log('[SCRAPER_V2] Routing requests to internal ingestion layer...');
  const allJobs = [];
  const seen = new Set();

  // We only hit the top 3 queries internally to stay lightning fast
  for (const q of queries.slice(0, 3)) {
    try {
      ...
      // Point to our internal Next.js API route
      // Using absolute URL if possible, or relative if on Vercel
      const baseUrl = process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : 'http://localhost:3000';
      const res = await fetch(`${baseUrl}/api/google-jobs?${params}`, {
      ...
      console.log(`[SCRAPER_V2] Internal API returned ${data.jobs_results?.length || 0} jobs for "${q.q || q}"`);
      ...
      for (const job of (data.jobs_results || [])) {
        ...
        allJobs.push({
          ...
          apply_url: job.share_link || `https://www.google.com/search?q=${encodeURIComponent(job.title + ' ' + job.company_name)}`,
          source: job.via || 'Google Jobs (Scraped)',
      ...
      console.error(`[SCRAPER_V2] Internal fetch failed:`, e.message);
    }
  }
...
export async function fetchAllJobs(profile, apiKeys = {}, onProgress, preferences = {}) {
  ...
  if (queries.length > 0) {
    const USE_INTERNAL_SERP = process.env.USE_INTERNAL_SERP === 'true';

    if (USE_INTERNAL_SERP) {
      const serpJobs = await fetchSerpInternal(queries, location);
      allJobs.push(...serpJobs);
      onProgress?.(`+${serpJobs.length} Scraped.`);
    } else {
